package com.shencoder.demo.grid;
...
//        mPagerChangedListener = null;
    ...
    private void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state, LayoutState layoutState, LayoutChunkResult layoutChunkResult) {
        ...
        layoutState.mCurrentPosition = layoutToEnd ? layoutState.getNextPosition(position,  mRows, mColumns, state) :
                layoutState.getPrePosition(position,  mRows, mColumns, state);
        ...
        layoutChunkResult.mConsumed = isNeedMoveSpan ? mItemWidth : 0;
    ...
    private void updateLayoutState(boolean layoutToEnd, int requiredSpace,
        ...
        mLayoutState.mCurrentPosition = layoutToEnd ? mLayoutState.getNextPosition(getPosition(child),  mRows, mColumns, state) :
                mLayoutState.getPrePosition(getPosition(child),  mRows, mColumns, state);
...
//                removeAndRecycleView(childAt, recycler);
    ...
    private void recycleViewsFromEnd(RecyclerView.Recycler recycler) {
        //如果clipToPadding==false，则不计算padding
        boolean clipToPadding = getClipToPadding();
        int end = clipToPadding ? getEndAfterPadding() : (getWidth());
        int childCount = getChildCount();
        for (int i = childCount - 1; i >= 0; i--) {
            View childAt = getChildAt(i);
            if (childAt != null) {
...
//                removeAndRecycleView(childAt, recycler);
    ...
    private int getDecoratedStart(View child) {
        final LayoutParams params = (LayoutParams) child.getLayoutParams();
        return getDecoratedLeft(child) - params.leftMargin;
    }
    ...
    private int computeScrollOffset(RecyclerView.State state) {
        ...
        int position = getPosition(firstView);
        final float avgSize = (float) getEnd() / (mColumns);
        int index;
        //所在第几列
        int pagerIndex = getPagerIndexByPosition(position);
    ...
    public PointF computeScrollVectorForPosition(int targetPosition) {
        ...
        if (DEBUG) {
            Log.w(TAG, "computeScrollVectorForPosition-firstSnapPosition: " + firstSnapPosition + ", targetPosition:" + targetPosition  + ", direction:" + direction);
        }
    ...
    protected static class LayoutState {
         ...
         * @return 下一个位置
         */
        protected int getNextPosition(int currentPosition, int rows, int columns, RecyclerView.State state) {
            int position;
         ...
         * @return 上一个位置
         */
        protected int getPrePosition(int currentPosition,  int rows, int columns, RecyclerView.State state) {
            int position;
            int onePageSize = rows * columns;
            int surplus = currentPosition % onePageSize;
            //水平滑动
